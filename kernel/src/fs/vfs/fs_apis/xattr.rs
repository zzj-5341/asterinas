...
fn clear_set_id_bits(inode: &dyn Inode) -> Result<()> {
    ...
    // Linux lets `CAP_FSETID` preserve both set-ID bits across content changes.
...
fn clear_set_id_bits_without_privilege(
    ...
    // Without `CAP_FSETID`, Linux retains a non-executable SGID bit only when
    // the caller belongs to the file's group (`setattr_should_drop_sgid()`).
