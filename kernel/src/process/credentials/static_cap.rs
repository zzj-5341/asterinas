...
use super::{
    Credentials, ExecCred, ExecPrivilegePolicy, FileCapabilities, Gid, SecureBits, Uid,
    capabilities::CapSet, credentials_::Credentials_,
};
use crate::prelude::*;
...
impl<R: TRights> Credentials<R> {
    ...
    pub(in crate::process) fn prepare_exec_cred(
        ...
        self.0
            .prepare_exec_cred(file_capabilities, setuid, setgid, privilege_policy)
    }
    ...
    /// This method requires the `Write` right.
    #[require(R > Write)]
    pub(in crate::process) fn apply_exec_cred(&self, exec_cred: ExecCred) {
