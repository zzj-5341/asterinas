...
impl Credentials_ {
    ...
    pub(super) fn prepare_exec_cred(
        ...
        let is_privileged_file =
            file_capabilities.is_some() || setuid.is_some() || setgid.is_some();
        let (permitted, effective, ambient) =
            self.calculate_capsets_for_exec(file_capabilities, exec_euid, is_privileged_file);
    ...
    fn calculate_capsets_for_exec(
        ...
        let ambient = if is_privileged_file {
            CapSet::empty()
        } else {
            self.ambient_capset()
        };
        ...
        (new_permitted, new_effective, ambient)
    }
