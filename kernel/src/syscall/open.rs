...
pub fn sys_open(path_addr: Vaddr, flags: u32, mode: u16, ctx: &Context) -> Result<SyscallReturn> {
    ...
    let file_handle = {
        let path = path.to_string_lossy();
        let fs_path = FsPath::from_fd_at(AT_FDCWD, path.as_ref(), EmptyPathStr::Reject)?;

        let fs_ref = ctx.thread_local.borrow_fs();
        let mask_mode = mode & !fs_ref.umask().get();

        let path_resolver = fs_ref.resolver().read();
        do_open_with_file_open_check(
            &path_resolver,
            &fs_path,
            flags,
            InodeMode::from_bits_truncate(mask_mode),
...
fn do_open_with_file_open_check(
    path_resolver: &PathResolver,
    fs_path: &FsPath,
    flags: u32,
    mode: InodeMode,
    ...
    if open_args.is_tmpfile() {
        return do_open_tmpfile_with_file_open_check(path_resolver, fs_path, &open_args);
    }
...
fn do_open_tmpfile_with_file_open_check(
    path_resolver: &PathResolver,
    fs_path: &FsPath,
    open_args: &OpenArgs,
) -> Result<Arc<dyn FileLike>> {
