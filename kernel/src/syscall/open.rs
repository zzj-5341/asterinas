...
fn do_open(
    ...
    // Validate incompatible creation flags before path resolution so later lookup errors do not mask EINVAL.
    ...
    let file_handle: Arc<dyn FileLike> = match lookup_res {
        LookupResult::Resolved(path) => {
            // Fix F: existing directory with O_CREAT|O_EXCL should be EEXIST
