...
static void load_profile(void)
{
	...
	policy_len =
		CHECK_WITH(snprintf(policy, sizeof(policy),
				    ...
				    CREATE_READ_ONLY_PATH, current_attr_path),
...
FN_SETUP(prepare_apparmor_environment)
{
	...
	struct stat statbuf;
	int stat_result = CHECK_WITH(stat(APPARMOR_LOAD_PATH, &statbuf),
				     _ret == 0 || errno == ENOENT);
	apparmor_enabled = stat_result == 0;
	if (!apparmor_enabled) {
		return;
	}
...
FN_TEST(profile_controls_read_access)
{
	...
	SKIP_TEST_IF(!apparmor_enabled || !open_syscall_supported());
...
FN_SETUP(cleanup_apparmor_environment)
{
	if (apparmor_enabled) {
		...
		CHECK_WITH(unlink(CREATE_ALLOWED_PATH),
			   _ret == 0 || errno == ENOENT);
